...
namespace TripOrganizer.API.Controllers
{
    ...
    public class TripsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateTrip(int id, [FromBody] UpdateTripModel model)
        {
            ...
            // Prevent modifying past trips
            if (trip.EndDate < DateTime.UtcNow)
                return BadRequest(new { message = "Cannot modify past trips" });

            // Only owners and co-owners can edit trips
            bool isOwner = trip.OwnerId == user.Id;
            bool isCoOwner = trip.CoOwners.Contains(user);
        ...
        public async Task<IActionResult> JoinTrip(int id)
        {
            ...
            // Prevent joining past trips
            if (trip.EndDate < DateTime.UtcNow)
                return BadRequest(new { message = "Cannot join past trips" });
        ...
        public async Task<IActionResult> LeaveTrip(int id)
        {
            ...
            // Owner cannot leave the trip
            if (trip.OwnerId == user.Id)
                return BadRequest(new { message = "The owner cannot leave the trip" });

            if (!trip.IsUserParticipant(user.Id))
                return BadRequest(new { message = "Not participating in this trip" });

            // First remove co-ownership if user is a co-owner
        ...
        public async Task<IActionResult> DeleteTrip(int id)
        {
            ...
            // Prevent deleting past trips
            if (trip.EndDate < DateTime.UtcNow)
                return BadRequest(new { message = "Cannot delete past trips" });
